int buttonValue1 = 0; // si les on a appuyé sur les boutons
int buttonValue2 = 0;
...
int greenLedValue1 = 0; //si le LED vert est allumé ou non 
int greenLedValue2 = 0;

int ledsArr[5] = {4,5,6,7,8}; // la list des led utilisée (sorties)
int delayValue; // la vitesse des led
int buttonPressedDelay = 2000;
int speedAdded = 20; // le valeur ajouté a cette vitesse
...
void setup() {
  ...
  pinMode(greenLed1, INPUT);// initialiser les leds verts come des entées
...
void loop() {
  ...
  while(gameIsStarted){
     for(int i = 0; i <= 4; i++){ // le cycle des leds
      ...
      digitalWrite(ledsArr[i], HIGH); //allumé le premier led
      delay(delayValue); // attends 250 ms
